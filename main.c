...
void systemset() // É utilizado para definir as configurações e os parametros inicais do jogo
{
    ...
    for (int i = 0; i < 3; i++)
    { // Limpa a matriz de jogo (define tudo como 2 novamente)
        for (int j = 1; j < 4; j++)
        {
            matriz[i][j] = 2;
        }
...
{
    if (totalJogadas == 8)
    { // Quando chegar em 8 jogadas, a função verifica se na proxima jogada pode haver uma vitória, caso sim, ele retorna 0
        for (int i = 0; i < 3; i++)
        {
            for (int j = 1; j < 4; j++)
            { // Verifica se na única posição livre é possível alguém vencer na proxima jogada
                if (matriz[i][j] == 2)
                {
                    if (quemComeca == usuario)
                    { // Verifica se o usuario pode ganhar na proxima jogada
                        matriz[i][j] = usuario;
                        if (!verificaVitoria())
                        {
                    ...
                    { // Verifica se o computador pode ganhar na proxima jogada
...
{
    char letra;
    if (representacaoJogador == usuario)
    { // Quando o usuário escolher que quer utilizar o tipo de jogador '0', faz a conversão da matriz númerica desta forma
        switch (numero)
        {
    ...
    else if (representacaoJogador == computador)
    { // Se o usuario escolher 'X', o número 1 é trocado por '0'
        switch (numero)
        {
        case usuario:
            letra = 'X';
...
void exibirJogo()
{ // Verifica se há alguma situação de vitoria ou empate e exibe as devidas "telas", do contrário, exibe o tabuleiro de jogo com base no proximo jogador
    system("cls");
    if (!verificaVitoria() && !verificaEmpate())
    {                              // Se não for vitoria nem empate, exibe o tabuleiro de jogo com base no proximo jogador
    ...
    else if (verificaVitoria() && jogadorVencedor == usuario)
    { // Caso seja vitoria e o usuario seja quem ganhou, exibe a mensagem de vitoria do usuario
        printf("*****************************************\n");
        printf("*                                       *\n");
        printf("*  ");
        printf("\033[0;32m");
    ...
    else if (verificaVitoria() && jogadorVencedor == computador)
    { // Caso seja vitoria, mas o vencedor seja o computador, exibe a mensagem de vitoria do computador
        printf("*****************************************\n");
        printf("*                                       *\n");
        printf("*  ");
        printf("\033[0;34m");
...
void usuarioJoga() // Usuario faz uma jogada
{
    ...
    while (1)
    { // Loop infinito para solicitar uma entrada válida
        char escolha;
        ...
        switch (escolha)
        {         // Escreve onde sera a linha e coluna da jogada, com base na resposta do usuario
        ...
        { // Verifica se a posição está ocupada e retorna para o usuario que a escolha não é possível
            printf("\033[0;31m");
...
void computadorJoga()
{
    ...
    for (int i = 0; i < 3; i++)
    { // Verifica se existe alguma jogada com possibilidade de vitoria
        for (int j = 1; j < 4; j++)
        {
            if (posicaoLivre(i, j))
            {                              // Se a posição em teste estiver livre, continua o if
                matriz[i][j] = computador; // Define a posição em teste para 3 (computador)
                if (verificaVitoria() && jogadorVencedor == computador)
                { // Testa se nessa jogada o computador é vencedor e caos seja faz a jogada
                    {
    ...
    for (int i = 0; i < 3; i++)
    { // Se não for possível ganhar, verifica se pode perder na proxima jogada
        for (int j = 1; j < 4; j++)
        {
            if (posicaoLivre(i, j))
            {                           // Se a posição em teste estiver livre, continua o if
                matriz[i][j] = usuario; // Define a posição em teste para 1 (usuario)
                if (verificaVitoria() && jogadorVencedor == usuario)
                {                              // Caso o usuario seja vitorioso nesta jogada, o computador bloqueia para não perder
                    matriz[i][j] = computador; // Faz a jogada
                    totalJogadas++;            // Adiciona 1 no total de jogadas
                    quemComeca = usuario;      // Proxima jogada é do usuario
                    return;                    // Termina a função após a jogada
    ...
    {
        if (peso[i] > valorMax)
        { // Verifica o maior peso do conjunto e caso o peso[0] não seja o maior, define o peso correspondente como maior peso
            valorMax = peso[i];
    ...
    switch (pos)
    {          // Faz a jogada com base na posição com maior peso
    case 0:    // Se o primeiro valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na primeira linha
        ...
        for (j = 1; j <= 3; j++)
        { // Verifica se alguma posição da primeira linha está livre
            if (posicaoLivre(i, j))
            {                              // Se a posição em teste estiver livre, continua o if
                matriz[i][j] = computador; // Faz a jogada
                totalJogadas++;            // Adiciona 1 no total de jogadas
    ...
    case 1: // Se o segundo valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na segunda linha
        ...
        {
            if (posicaoLivre(i, j))
            {                              // Verifica se alguma posição da segunda linha está livre
                matriz[i][j] = computador; // Faz a jogada
                totalJogadas++;            // Adiciona 1 no total de jogadas
                quemComeca = usuario;      // Proxima jogada é do usuario
                return;                    // Termina a função após a jogada
    ...
    case 2: // Se o terceiro valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na terceira linha
        ...
        {
            if (posicaoLivre(i, j))
            {                              // Verifica se alguma posição da terceira linha está livre
                matriz[i][j] = computador; // Faz a jogada
                totalJogadas++;            // Adiciona 1 no total de jogadas
                quemComeca = usuario;      // Proxima jogada é do usuario
                return;                    // Termina a função após a jogada
    ...
    case 3: // Se o quarto valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na diagonal principal
        if (posicaoLivre(0, 1))
        {                              // Verifica se a primeira posição da diagonal principal está livre
        ...
        if (posicaoLivre(1, 2))
        {                              // Verifica se a segunda posição da diagonal principal está livre
        ...
        if (posicaoLivre(2, 3))
        {                              // Verifica se a terceira posição da diagonal principal está livre
    ...
    case 4: // Se o quinto valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na terceira coluna
        ...
        {
            if (posicaoLivre(i, j))
            {                              // Verifica se alguma posição da segunda coluna está livre
                matriz[i][j] = computador; // Faz a jogada
                totalJogadas++;            // Adiciona 1 no total de jogadas
                quemComeca = usuario;      // Proxima jogada é do usuario
                return;                    // Termina a função após a jogada
    ...
    case 6: // Se o sétimo valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na terceira coluna
        ...
        {
            if (posicaoLivre(i, j))
            {                              // Verifica se alguma posição da primeira coluna está livre
                matriz[i][j] = computador; // Faz a jogada
                totalJogadas++;            // Adiciona 1 no total de jogadas
                quemComeca = usuario;      // Proxima jogada é do usuario
                return;                    // Termina a função após a jogada
    ...
    case 7: // Se o oitavo valor do conjunto peso[8] for o maior, faz a jogada em alguma posição livre na diagonal segundaria
        if (posicaoLivre(0, 3))
        {                              // Verifica se a primeira posição da diagonal segundaria está livre
        ...
        if (posicaoLivre(1, 2))
        {                              // Verifica se a segunda posição da diagonal segundaria está livre
            matriz[1][2] = computador; // Faz a jogada
            totalJogadas++;            // Adiciona 1 no total de jogadas
            quemComeca = usuario;      // Proxima jogada é do usuario
            return;                    // Termina a função após a jogada
        ...
        if (posicaoLivre(2, 1))
        {                              // Verifica se a terceira posição da diagonal segundaria está livre
...
/* PRIMEIRA VERSÃO DA JOGADA POR PESOS (SÓ UTILIZAVA OS PESOS DE COLUNA, PORTANTO NÃO ERA TÃO PRECISO)

int melhorPeso;                // Cria a variavel responsavel por guardar a melhor pontuação possivel
int melhorLinha, melhorColuna; // Cria as variaveis responsaveis por guardar a melhor posição de jogada
for (int i = 0; i < 3; i++)
{ // Se nenhuma das condições acima forem verdadeiras, a jogada é feita com base nos pesos
    for (int j = 1; j < 4; j++)
    {
        if (posicaoLivre(i, j))
        {                              // Se a posição sendo testada for 2 (posição livre), é iniciada a verificação do melhor peso
            matriz[i][j] = computador; // Define a posição em teste para 3 (computador)
            calculoMatriz();           // Calcula os pesos da matriz após a jogada de teste
            if (matriz[3][j] > melhorPeso)
            {                              // Verifica se o peso da coluna sendo testada é maior que o melhor peso salvo,
                melhorPeso = matriz[3][j]; // Caso o peso testado seja maior, guarda o valor em 'melhorPeso'
                melhorLinha = i;           // Melhor Linha para jogada é = a linha testada
                melhorColuna = j;          // Melhor Coluna para jogada é = a coluna testada
            }
            matriz[i][j] = 2; // Desfaz a jogada de verificação após a verificação
        }
    }
}
matriz[melhorLinha][melhorColuna] = computador;
totalJogadas++;
quemComeca = usuario; // Proxima jogada é do usuario */
...
void iniciaJogo() // Inicia o jogo
{
    ...
    while (!escolhaValida)
    { // Faz um loop até que uma escolha válida seja feita
        printf("*****************************************\n");
        printf("*                                       *\n");
        printf("*  ");
        printf("\033[0;31m");
        ...
        { // Define a variavel 'representacaoJogador', convertendo posteriormente no tabuleiro para a escolha do usuário
        case 'X':
        ...
        case 'o':
            ...
            break;
        default:
        { // Exibe a mensagem de erro caso a entrada não seja x ou o
            printf("\033[0;31m");
            printf("\nOpção inválida! ");
            printf("\033[0;37m");
    ...
    { // Faz um loop até que uma escolha válida seja feita
        ...
        switch (escolhaQuemComeca)
        { // Define a variavel 'escolhaQuemComeca', convertendo posteriormente no tabuleiro para a escolha do usuário
        case 1:
        ...
        case 2:
            ...
            break;
        default:
        { // Exibe a mensagem de erro caso a entrada não seja 1 ou 2
            printf("\033[0;31m");
            printf("\nOpção inválida! ");
            printf("\033[0;37m");
            printf("Por favor, insira novamente.\n\n");
...
void jogadas()
{ // Faz as jogadas até o jogo acabar
    while (quemComeca != 0)
    { // Se quem começa não for 0, o jogo continua
        if (!verificaVitoria())
        { // Se não tiver vitoria, o jogo continua
            if (!verificaEmpate())
            { // Se não tiver empate o jogo continua
                if (quemComeca == usuario)
                {                  // Se 'quemComeça' for o usuario, chama a função 'usuarioJoga'
                    exibirJogo();  // Exibe o tabuleiro
                    usuarioJoga(); // Exibe o tabuleiro de jogada e computa a jogada do usuario
                }
                else if (quemComeca == computador)
                {                     // Se 'quemComeça' for o computador, chama a função 'computadorJoga'
                    exibirJogo();     // Exibe o tabuleiro
...
void jogarNovamente()
{ // Ao fim do jogo, pergunta se o usuario deseja jogar novamente
    system("cls");
    escolhaValida = 0;             // Limpa o valor de 'escolhaValida'
    ...
    { // Faz um loop até que uma escolha válida seja feita
        ...
        switch (escolhaJogarNovamente)
        { // Swith responsavel por quebrar o While
        case 1:
        ...
        case 2:
            ...
            break;
        default:
        { // Exibe a mensagem de erro caso a entrada do usuario seja invalida
            printf("\033[0;31m");
            printf("\nOpção inválida! ");
            printf("\033[0;37m");
            printf("Por favor, insira novamente.\n\n");
    ...
    {                     // Se a escolha do usuario for jogar novamente, inicia o jogo mais uma vez
